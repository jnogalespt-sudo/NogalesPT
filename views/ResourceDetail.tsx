        ...
        <div className="animate-pulse space-y-8 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="aspect-video bg-slate-200 rounded-[40px]"></div>
            </div>
            <div className="space-y-8">
              <div className="bg-white p-10 rounded-[48px] shadow-sm border border-slate-100 text-center space-y-8">
                <div className="w-28 h-28 bg-slate-200 rounded-[36px] mx-auto"></div>
                <div className="h-6 bg-slate-200 rounded-md w-1/2 mx-auto mt-6"></div>
                <div className="h-14 bg-slate-200 rounded-[24px] w-full"></div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="animate-in fade-in duration-300 space-y-8">
      ...
      {selectedResource.kind === 'blog' ? (
        <article className="max-w-[850px] mx-auto bg-white rounded-[48px] shadow-sm border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-8 duration-700">
          <div className="h-[450px] w-full relative bg-slate-900">
            {isLoading && (
              <div className="absolute inset-0 bg-slate-800 animate-pulse"></div>
            )}
            {!isLoading && (
              <img loading="lazy" src={selectedResource.thumbnail} className="absolute inset-0 w-full h-full object-cover" alt={selectedResource.title} />
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end p-12 md:p-20 z-10">
          ...
          <div className="p-12 md:p-20">
            <div className="flex items-center gap-4 mb-16 pb-12 border-b border-slate-50">
              <img loading="lazy" src={users.find(u => u.email === selectedResource.email)?.avatar || `https://ui-avatars.com/api/?name=${selectedResource.authorName}`} className="w-16 h-16 rounded-[22px] shadow-xl object-cover border-4 border-white" alt={selectedResource.authorName} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-8">
            <div className="bg-white p-10 rounded-[48px] shadow-sm border border-slate-100 text-center space-y-8 sticky top-24">
              ...
              {isLoading ? (
                <div className="h-14 bg-slate-200 rounded-[24px] w-full animate-pulse"></div>
              ) : (
